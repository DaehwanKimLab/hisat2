...
##        print(item)
    ...
    for allele,varList in cyp2d6_var_dict.items():
        for var in varList:
            ...
            try:
                assert(cyp2d6_seq[pos - 1] == ntChange[0]) # nt at pos in seq must match database
            except:
                print >> sys.stdout, "Warning: position %d in sequence contains %s, but expected %s from database" % (pos, cyp2d6_seq[pos - 1], ntChange[0])
