...
import csv, os, sys, math, random, re
...
def simulate_reads(genome_file, gtf_file, snp_file, base_fname,
                   rna, methyl_profile, paired_end, read_len, frag_len,
    ...
    [meth_cglocs,meth_cgstra,meth_cgmpro,meth_cgkeys] = [[],[],[],[]]
...
def genRandomMethylationProfile(genome_dic,locflag,ofilename):
	cglocs = [];
	cgstra = [];
	cgmpro = [];
	cgkeys = []; 

	for k in genome_dic.keys():
		for match in re.finditer(locflag,genome_dic[k]):
			cglocs.append(match.span()[0])
			cgstra.append("+")
	...
	with open(ofilename,'wt') as out_file:
		...
		for i in range(len(cglocs)):
			tsv_writer.writerow([cgkeys[i],cglocs[i],cgstra[i],cgmpro[i]])
	
	return(cglocs,cgstra,cgmpro,cgkeys)

def readMethylationProfile(ifilename):
	cglocs = [];
	cgstra = [];
	cgmpro = [];
	cgkeys = []; 
	...
	with open(ifilename,'r') as in_file: 
		...
		for line in in_file: 
			...
			cgkeys.append(linepieces[0])
			cglocs.append(linepieces[1])
			cgstra.append(linepieces[2])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--methylate',
                        dest="meth",
                        action='store',
                        type=str,
                        default='N',
                        help='methylate reads according to <methylation profile - default generate random>')
    ...
    simulate_reads(args.genome_file, args.gtf_file, args.snp_file, args.base_fname,
                   args.rna, args.meth, args.paired_end, args.read_len, args.frag_len,
