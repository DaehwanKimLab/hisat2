                                          ...
                                          index_t                     dep)
{
    ...
    if(left) {
        ...
        for(int rf_i = (int)rflen - 1; rf_i >= 0 && min_rd_i >= 0; rf_i--, min_rd_i--) {
            int rf_bp = rfseq[rf_i];
            int rd_bp = rdseq[min_rd_i];
            if(rf_bp != rd_bp) {
                tmp_mm++;
                if(tmp_mm > mm) break;
                if(edits != NULL) {
        ...
        for(; alt_range.second > alt_range.first; alt_range.second--) {
            ...
            if(orig_nedits < edits->size()) edits->erase(0, edits->size() - orig_nedits);
        }
        return 0;
    } else {
        index_t tmp_mm = 0;
        index_t max_rd_i = 0;
        for(index_t rf_i = 0; rf_i < rflen && max_rd_i < rdlen; rf_i++, max_rd_i++) {
            int rf_bp = rfseq[rf_i];
            int rd_bp = rdseq[rdoff + max_rd_i];
            if(rf_bp != rd_bp) {
                tmp_mm++;
                if(tmp_mm > mm) break;
                if(edits != NULL) {
