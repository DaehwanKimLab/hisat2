...
struct SideLocus {
	 ...
	 */
	void initFromRow(index_t row, const GFMParams<index_t>& ep, const uint8_t* ebwt) {
	 ...
	 */
	void nextSide(const GFMParams<index_t>& ep) {
		assert(valid());
...
public:
	...
	// I/O
	void readIntoMemory(int color, int needEntireRev, bool loadSASamp, bool loadFtab, bool loadRstarts, bool justHeader, GFMParams<index_t> *params, bool mmSweep, bool loadNames, bool startVerbose);
...
#define WITHIN_FCHR_DOLLARA(x) \
	...
	bool inMemoryRepOk(const GFMParams<index_t>& gh) const {
		...
		assert_lt(_zEbwtBpOff, 4);
		assert_lt(_zEbwtByteOff, gh._ebwtTotSz);
